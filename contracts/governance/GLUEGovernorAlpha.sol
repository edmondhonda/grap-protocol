...
// Modified to work in the GLUE system

// all votes work on underlying _glueBalances[address], not balanceOf(address)
...
//
// Changes made by GLUE after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since GLUE supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match GLUE as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "GLUE Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(glue.initSupply(), 4), 100); } // 4% of GLUE

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(glue.initSupply(), 100); } // 1% of GLUE
    ...
    constructor(address timelock_, address glue_) public {
        timelock = TimelockInterface(timelock_);
        glue = GLUEInterface(glue_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(glue.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || glue.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = glue.getPriorVotes(voter, proposal.startBlock);
